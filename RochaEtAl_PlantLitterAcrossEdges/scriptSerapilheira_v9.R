...
# In which side was there more of each fraction per month?
...
### Monthly model selection figures

newdists <- -150:150

for(i in 1:12) {
	print(paste(i, "of 12"))
	year <- ifelse(i<5, 2017, 2018)
	dadost.i <- subset(dadost, Month==months[[i]])
	setwd("e:/Pavel/Profissional/Pesquisa/MyPapers_Working/Janaine_2019_EILitterUna/v8_PlantEcol/figs_SM/")
	png(filename=paste(formatC(i,digits=1, flag=0),"_",
		months[[i]],".png",sep=""), height=20, width=20, unit="cm", res=300)
	par(mfrow=c(2,2), mar=c(3,3,2,2), oma=c(2,2,2,2))
	mod.leaves <- bestModels[i,1]
	if(mod.leaves == "cat_edgeAsBurnt") {
		plot(Mass_leaves ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="a. Leaf litter")
		abline(v=0, col="gray", lty=3, lwd=2)
		segments(x0=-150, x1=5, 
			y0=mean(subset(dadost.i, Side_edgeAsBurnt=="Burnt")$Mass_leaves), 
			y1=mean(subset(dadost.i, Side_edgeAsBurnt=="Burnt")$Mass_leaves),
			col="black")
		segments(x0=5, x1=150, 
			y0=mean(subset(dadost.i, Side_edgeAsBurnt=="Forest")$Mass_leaves), 
			y1=mean(subset(dadost.i, Side_edgeAsBurnt=="Forest")$Mass_leaves), 
			col="black")
	}
	if(mod.leaves=="gam") {
		mod.leaves.gam <- gamm4(Mass_leaves ~ s(Distance, fx=F, k=4), 
			random=~(1|Transect), data=dadost.i, family=Tweedie(fam))
		predicted <- predict(mod.leaves.gam$gam,
			newdata=list(Distance = newdists), type="response")
		plot(Mass_leaves ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="a. Leaf litter")
		abline(v=0, col="gray", lty=3, lwd=2)
		lines(predicted ~ newdists, col="black")
	}
	mod.fwd <- bestModels[i,2]
	if(mod.fwd == "null") {
		plot(Mass_fwd ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="a. FWD")
		abline(v=0, col="gray", lty=3, lwd=2)
		segments(x0=-150, x1=150, 
			y0=mean(dadost.i$Mass_fwd), 
			y1=mean(dadost.i$Mass_fwd),
			col="black")
	}
	if(mod.fwd=="cat_edgeAsForest") {
		plot(Mass_fwd ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="b. FWD")
		abline(v=0, col="gray", lty=3, lwd=2)
		segments(x0=-150, x1=-5, 
			y0=mean(subset(dadost.i, Side_edgeAsForest=="Burnt")$Mass_fwd), 
			y1=mean(subset(dadost.i, Side_edgeAsForest=="Burnt")$Mass_fwd),
			col="black")
		segments(x0=-5, x1=150, 
			y0=mean(subset(dadost.i, Side_edgeAsForest=="Forest")$Mass_fwd), 
			y1=mean(subset(dadost.i, Side_edgeAsForest=="Forest")$Mass_fwd), 
			col="black")
	}
	mod.misc <- bestModels[i,3] 
	if(mod.misc == "null") {
		plot(Mass_misc ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="c. Miscellaneous")
		abline(v=0, col="gray", lty=3, lwd=2)
		segments(x0=-150, x1=150, 
			y0=mean(dadost.i$Mass_misc), 
			y1=mean(dadost.i$Mass_misc),
			col="black")
	}
	mod.total <- bestModels[i,4]
	if(mod.total == "cat_edgeAsBurnt") {
		plot(Mass_total ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="d. Total litter")
		abline(v=0, col="gray", lty=3, lwd=2)
		segments(x0=-150, x1=5, 
			y0=mean(subset(dadost.i, Side_edgeAsBurnt=="Burnt")$Mass_total), 
			y1=mean(subset(dadost.i, Side_edgeAsBurnt=="Burnt")$Mass_total),
			col="black")
		segments(x0=5, x1=150, 
			y0=mean(subset(dadost.i, Side_edgeAsBurnt=="Forest")$Mass_total), 
			y1=mean(subset(dadost.i, Side_edgeAsBurnt=="Forest")$Mass_total), 
			col="black")
	}
	if(mod.total=="cat_edgeAsForest") {
		plot(Mass_total ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="d. Total litter")
		abline(v=0, col="gray", lty=3, lwd=2)
		segments(x0=-150, x1=-5, 
			y0=mean(subset(dadost.i, Side_edgeAsForest=="Burnt")$Mass_total), 
			y1=mean(subset(dadost.i, Side_edgeAsForest=="Burnt")$Mass_total),
			col="black")
		segments(x0=-5, x1=150, 
			y0=mean(subset(dadost.i, Side_edgeAsForest=="Forest")$Mass_total), 
			y1=mean(subset(dadost.i, Side_edgeAsForest=="Forest")$Mass_total), 
			col="black")
	}
	if(mod.total=="gam") {
		mod.total.gam <- gamm4(Mass_total ~ s(Distance, fx=F, k=4), 
			random=~(1|Transect), data=dadost.i, family=Tweedie(fam))
		predicted <- predict(mod.total.gam$gam,
			newdata=list(Distance = newdists), type="response")
		plot(Mass_total ~ Distance, data=dadost.i, pch=16, 
			col=alpha("black",0.5), xlab="", ylab="", main="d. Total litter")
		abline(v=0, col="gray", lty=3, lwd=2)
	...
	mtext(side=1, text="Distance (m)", outer=TRUE)
	mtext(side=2, text="Monthly litterfall (t/ha)", outer=TRUE)
	mtext(side=3, text=paste(formatC(i,digits=1,flag=0), "-", months[[i]],
		year, sep=" "), outer=TRUE)
	dev.off()
}
...
png(filename="figure3_GAM_forest_v8.png", height=20, 
...
legend("topright", legend=paste("p=", formatC(p.gam.leaves.forest, digits=2,
	format="g"), sep=""), bty="n")
...
legend("topright", legend=paste("p=", formatC(p.gam.fwd.forest, digits=2,
	format="g"), sep=""), bty="n")
...
legend("topright", legend=paste("p=", formatC(p.gam.misc.forest, digits=2,
	format="g"), sep=""), bty="n")
plot(Mass_total ~ Distance, data=dados.sum.forest, pch=16, 
	col=alpha("black",0.5), xlab="", ylab="", main="d. Total")
legend("topright", legend=paste("p=", formatC(p.gam.total.forest, digits=2,
	format="g"), sep=""), bty="n")
...
setwd("e:/Pavel/Profissional/Pesquisa/MyPapers_Working/Janaine_2019_EILitterUna/v8_PlantEcol/figs_SM2/")
...
for(i in 1:12) {
	...
	plot(Mass_misc ~ Distance, data=dados.i.burnt, pch=16, 
		bg=alpha("black", 0.5), xlab="", ylab="", 
		main="Burnt forest - miscellaneous",
	...
	plot(Mass_misc ~ Distance, data=dados.i.forest, pch=16, 
		bg=alpha("black", 0.5), xlab="", ylab="", 
		main="Unburnt forest - miscellaneous",
		ylim=c(0,max(dados.i$Mass_misc)))
